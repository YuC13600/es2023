...
(defrule define-uncle
    (query (name ?p1) (subject ?p2))
    (parent-children (parent $? ?temppa $?)
        (children $? ?p2 $?))
    (parent-children (parent $? ?tempgr $?)
        (children $? ?temppa $?))
    (pair (husband ?p1) (wife ?temppair))
    (or
        (or (parent-children (parent $? ?tempgr $?)
            (children $? ?p1 $? ?temppa $?))
            (parent-children (parent $? ?tempgr $?)
            (children $? ?temppair $? ? temppa $?))
        )
        (or (parent-children (parent $? ?tempgr $?)
            (children $? ?temppa $? ?p1 $?))
            (parent-children (parent $? ?tempgr $?)
            (children $? ?temppa $? ?temppair $?))
        )
    )
...
(defrule define-cousin
    (query (name ?p1) (subject ?p2))
    (parent-children (parent $? ?temppap2 $?)
        (children $? ?p2 $?))
    (parent-children (parent $? ?tempgr $?)
        (children $? ?temppap2 $?))
    (parent-children (parent $? ?temppap1 $?)
        (children $? ?p1 $?))
    (parent-children (parent $? ?tempgr $?)
        (children $? ?temppap1 $?))
    (person (name ?tempgr) (sex male))
    (or (parent-children (parent $? ?tempgr $?)
        (children $? ?temppap1 $? ?temppap2 $?))
        (parent-children (parent $? ?tempgr $?)
        (children $? ?temppap2 $? ?temppap1 $?))
    )
    =>
